...
% dimx = (1,7) contains the length of f(:),z(:),diagS,ReS,ImS,Se,phi
dimx = [m,m,m,m*(m-1)/2,m*(m-1)/2,1,n*m]; % (1,6)
nx = sum(dimx(:)); % no. of parameters in hessian
  
% calc. hessian using analytical hessian
fprintf('by analytical formula ... ');
fprintf('fastest form ... ');
[hmat, gvec] = myfun_hess(f,z,S,Se,BA1,alph,ff,F,dimx);

% --------------------------------------------------------------------------
% sqr_Se = sqrt(Se);
% sqr_iS = chol(S,'lower')^-1;
% P = [f; z; phi; sqr_iS; sqr_Se*ones(1,m)];
% [hmat2,gvec2] = hessEM(F,ffT,P,d,A0);
% 
% fun = @(x)oneloglik(Fki,fki,x,d);
% [grad,err,finaldelta] = gradest(fun,P);
% 
% fun = @(x)Q2(x,[f z],ffT,sqr_iS*sqr_iS',EQk2)
% [grad,err,finaldelta] = gradest(fun,ones(1,6));

% --------------------------------------------------------------------
% compute inverse through spectral representation to by-pass singularity
% due to mode shape scaling (i.e., along the dir. of alph)
% entries correspond to a mean of [1,1,1,phi,1]
[vv,dd]=eig(hmat);
[dd,I]= sort(diag(dd)); % eigenvalues in ascending order
vv = vv(:,I);
nnpos = length(find(dd<=0)); % nnpos=no. of non-positive eig values
I = [max(nnpos,m)+1:nx]; % index of eigen modes to retain
cmat = vv(:,I)*diag(1./dd(I))*vv(:,I).';

fprintf('done\n');

fprintf('No. of non-positive eigenvalues of Hessian: %2.0f\n',nnpos);
fprintf('No. of principal directions excluded from calculating inverse: %2.0f\n',max(nnpos,m));
fprintf('Eigenvalues of Hessian:');
fprintf(['\n',repmat('%1.1e ',[1 10])],dd);
fprintf('\n');
if nnpos>m
    disp('Warning: No. of non-positive eigenvalues > no. of modes. A sign of unidentifiability or prediction error at boundary 0.');
    disp('Warning: Posterior c.o.v. of Se and phi may be incorrect.');
    fprintf('Excluded principal directions ({f(:),z(:),S(:),V}, {phi(:,i):i=1,...,n}):')
    for jj=1:nnpos
        fprintf(['\n',repmat('%1.1e ',[1 nx-n*m])],vv(1:nx-n*m,jj));
        for jj2=1:m
            fprintf(['\n',repmat('%1.1e ',[1 n])],vv(nx-n*m+(jj2-1)*n+[1:n],jj));
        end
    end
    fprintf('\n');
end
...
% stdv.
stdv = sqrt(diag(cmat)).'; % col. vector

% c.o.v.
coefv = stdv; % since hmat is calc. wrt mpv; see myfun_hess
coefv = mat2cell(coefv,1,dimx); % {1,7} cell

% structure cmat into a cell
cmat = mat2cell(cmat,dimx,dimx); % {6,6} cell

% return sqrt. sum of principal var. for mode shape uncertainty
if dimx(7) > 0
    coefv{7} = zeros(1,m);
    for ii=1:m
        I = (ii-1)*n + [1:n];
        coefv{7}(ii) = sqrt(trace(cmat{7,7}(I,I)));
    end
else
    coefv{7} = zeros(0,m);
end
...
% expand Se to all modes
coefv{4} = repmat(coefv{4},[1,m]); % (1,m)

%   % coefv. of rms basd on coefv of f,z,Sii
%   rmscov = coefv{1}.^2 + coefv{2}.^2 + coefv{3}.^2 ...
%                - 2*diag(cmat{1,2}).' + 2*diag(cmat{1,3}).' ...
%                - 2*diag(cmat{2,3}).'; % (1,m), c.o.v.^2 of rms^2
%   rmscov = sqrt(rmscov)/2; % (1,m), c.o.v. of rms

% coefv. of rms basd on coefv of f,z,Sii
rmscov = ...
    coefv{1}.^2 + coefv{2}.^2 + coefv{3}.^2 ...
    - 2*diag(cmat{1,2}).' + 2*diag(cmat{1,3}).' ...
    - 2*diag(cmat{2,3}).'; % (1,m), c.o.v.^2 of rms^2
rmscov = sqrt(rmscov)/2; % (1,m), c.o.v. of rms
